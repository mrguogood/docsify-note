...
-  不同字符集导致索引失败，建议utf8mb4
  ...
  [MySQL高级篇——索引失效的11种情况](https://blog.csdn.net/qq_40991313/article/details/130779528)
...
- 在筛选字段加索引
- 将子查询，提前定义为一张表，用with,也就是临时表![img](./../images/28641981_630d1140-16ac-4d0f-bc7a-cdb8d3807bdc.png)
- 对临时表执行速度慢的进行调整，可以通过关联主表进行提前筛选
- 注意关联表的顺序 
  - **在数据库执行 `大表 LEFT JOIN 小表` 时，数据库通常会先扫描大表（即使小表没有匹配的记录，大表的所有行也会被保留），然后再去查找小表中匹配的行。如果后续还有 `INNER JOIN` 操作，这个过程可能会产生大量中间数据，导致查询变慢。**![image-20260130164801981](./../images/image-20260130164801981.png)![image-20260130164747299](./../images/image-20260130164747299.png)![image-20260130164722885](./../images/image-20260130164722885.png)![image-20260130164656242](./../images/image-20260130164656242.png)
- 实战
  - ![image-20260130164937777](./../images/image-20260130164937777.png)